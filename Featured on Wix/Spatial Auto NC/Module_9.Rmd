...
output:
  ...
  pdf_document:
    toc: yes
    toc_depth: '5'
subtitle: Manufacturing Jobs in NC 1990-2000
---
<hr>
...
**Creating Nearest Neighbor Spatial Objects**
...
queens_case_plot <- 
  ...
  #plotting the borders of our nc counties  
  tm_shape(NC_UTM) +
  tm_borders() +
...
tm_shape(nb_lines) + 
  tm_lines() +
#adding centroid dot for NC counties  
...
**Creating wij Weight Matrix Using the above Queen's Case Spacial Object**
...
#Summary Statistics of Our Weights matrix
summary(queen_nb_w)
```

## Testing for Spatial Autocorrelation using Morans's I
...
Calculating Moran's I & p-values for Different Variables 
...
# Rooks, knn, and distance based Neighest Neighbor
...
**Bringing in code from previous module:** so that we have the `NC_knn1` object which I've tweaked to call `NC_knn4`, and the `NC_dnn` object.

### Rooks 
Spatial Weights Matrix & Testing Morans I
...
nb_lines_r <- nb2lines(nb = rook_nb,
                     coords = nc_coords)
...
rooks_case_plot <- 
  ...
  tm_shape(NC_UTM) +
  tm_borders() +
 tm_layout(title= 'Rooks Case', title.position = c('left', 'top')) +
#adding nb lines
tm_shape(nb_lines_r) + 
  tm_lines() +
#adding centroid dot for NC counties  
...
# Local Moran's I (LISA) Queens Case
